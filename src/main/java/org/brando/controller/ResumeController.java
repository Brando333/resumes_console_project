...
public class ResumeController {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        resumeList.add(new Resume(
                ...
                address,
                description,
                toList(rawCertificationsString),
                toMap(rawSocialNetworksString),
                toList(rawSkillsString),
                user_id)
        );
      }
  ...
  private static List<String> toList(String raw) {
    ...
    return Stream.of(cleaned.split(", ")).filter(i -> !i.isEmpty()).collect(Collectors.toList());
  }
  ...
  private static Map<String, String> toMap(String raw) {
    ...
    String cleaned = raw.substring(1, raw.length() - 1);
    return Arrays.stream(cleaned.split(", ")).map(kv -> kv.split("=")).filter(kv -> kv.length == 2) //just to assure we have a key and value in each array, and no enter incorrect data to the db
